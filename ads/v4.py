...
class ST7789V3:
    ...
    def _reset(self):
        self._rst.set_value(RST_LINE, gpiod.line.Value.INACTIVE)
        time.sleep(0.1)
        self._rst.set_value(RST_LINE, gpiod.line.Value.ACTIVE)
        time.sleep(0.2)
    ...
    def _init(self):
        self._cmd(0x01)
        time.sleep(0.15)  # software reset
        self._cmd(0x11)
        ...
        self._cmd(0x36)
        self._data([0x60])  # 90 degrees clockwise
        # self._cmd(0x36); self._data([0xC0])  # 180 degrees
        # self._cmd(0x36); self._data([0xA0])  # 270 degrees

        self._cmd(0xB2)
        self._data([0x0C, 0x0C, 0x00, 0x33, 0x33])  # PORCTRL
        self._cmd(0xB7)
        self._data([0x35])  # GCTRL
        self._cmd(0xBB)
        self._data([0x19])  # VCOMS
        self._cmd(0xC0)
        self._data([0x2C])  # LCMCTRL
        self._cmd(0xC2)
        self._data([0x01])  # VDVVRHEN
        self._cmd(0xC3)
        self._data([0x12])  # VRHS
        self._cmd(0xC4)
        self._data([0x20])  # VDVS
        self._cmd(0xC6)
        self._data([0x0F])  # FRCTRL2
        self._cmd(0xD0)
        self._data([0xA4, 0xA1])  # PWCTRL1

        self._cmd(0xE0)
        self._data([  # PVGAMCTRL
            0xD0, 0x04, 0x0D, 0x11, 0x13, 0x2B, 0x3F, 0x54, 0x4C, 0x18, 0x0D,
            0x0B, 0x1F, 0x23
        ])
        self._cmd(0xE1)
        self._data([  # NVGAMCTRL
            0xD0, 0x04, 0x0C, 0x11, 0x13, 0x2C, 0x3F, 0x44, 0x51, 0x2F, 0x1F,
            0x1F, 0x20, 0x23
        ])
    ...
    def _set_window(self, x0=0, y0=0, x1=None, y1=None):
        ...
        ye = y1 + Y_OFFSET
        self._cmd(0x2A)
        self._data([(xs >> 8) & 0xFF, xs & 0xFF, (xe >> 8) & 0xFF, xe & 0xFF])
        self._cmd(0x2B)
        self._data([(ys >> 8) & 0xFF, ys & 0xFF, (ye >> 8) & 0xFF, ye & 0xFF])
        self._cmd(0x2C)
...
def _convert_and_enqueue(src: Path, dst: Path,
                         q: "queue.Queue[Optional[Path]]") -> None:
...
def schedule_conversions(sources: List[Path]) -> "queue.Queue[Optional[Path]]":
    ...
    for src in sources:
        ...
        if dims == (TARGET_W, TARGET_H):
            print(
                f"[skip] {src.name} is already {TARGET_W}x{TARGET_H}, copying")
...
def demo_shapes(device):
    ...
    draw.polygon([(120, 230), (70, 140), (170, 140)],
                 outline="white",
                 fill="teal")
...
def my_videos(device):
    ...
    ready: List[Path] = []  # fully converted, ready to play
    remaining = len(sources)  # total expected from queue
    idx = 0  # current position in ready list

    while remaining > 0 or idx < len(ready):
        # drain everything currently in the queue (non-blocking)
        while True:
            try:
                result = play_queue.get_nowait()
                remaining -= 1
                if result is None:
                    print("  x a conversion failed, skipping slot")
                else:
                    print(f"  + ready: {result.name}")
                    ready.append(result)
            except queue.Empty:
                break

        if ready:
            path = ready[idx % len(ready)]
            idx += 1
            print(f"\n> {path.name}")
            play_video(device, path, fps=24)

            # if we've cycled through all ready clips and more are pending,
            # do one blocking wait so we don't busy-spin
            if idx % len(ready) == 0 and remaining > 0:
                result = play_queue.get()
                remaining -= 1
                if result is None:
                    print("  x a conversion failed, skipping slot")
                else:
            ...
            # nothing ready yet — block until first clip arrives
            result = play_queue.get()
            remaining -= 1
            if result is None:
                print("  x a conversion failed, skipping slot")
